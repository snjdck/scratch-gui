...
function genCode(runtime, target, id){
    ...
    while(id != null){
        ...
        if(!runtime.getIsHat(opcode)){
            result.push(...genCodeTree(runtime, target, id, 0));
        }
...
function genCodeTree(runtime, target, id, retCount){
    var blockContainer = target.blocks;
    var block = blockContainer.getBlock(id);
    if(block == null){
        blockContainer = runtime.flyoutBlocks;
    ...
    for(let key in fields){
        ...
        if(field.block)
            argValues[key] = genCodeTree(runtime, target, field.block, 1)[0];
        else
            argValues[key] = SyntaxTreeFactory.NewString(field.value);
    }
    for(let key in inputs){
        let input = inputs[key];
        if(input.block)
            argValues[key] = genCodeTree(runtime, target, input.block, 1)[0];
    }
    ...
    return [SyntaxTreeFactory.Call(opcode, argValues, retCount)];
}
